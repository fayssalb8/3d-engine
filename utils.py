...
def convert_stl_to_sla(input_path, output_path, params, slicer_path):
    """
    Convert STL file to SLA/DLP format using PrusaSlicer

    Args:
        input_path: Path to input STL file
        output_path: Path to output .sl1 file
        params: Dictionary containing slicing parameters
        slicer_path: Path to PrusaSlicer executable
    ...
    try:
        ...
        # Build PrusaSlicer SLA command
        cmd = [
            slicer_path,
            '--export-sla',
            input_path,
            '--output', output_path,
            '--layer-height', str(params.get('layer_height', 0.05)),
        ]

        # Add SLA-specific parameters if provided
        if params.get('exposure_time'):
            cmd.extend(['--exposure-time', str(params.get('exposure_time'))])
        if params.get('initial_exposure_time'):
            cmd.extend(['--initial-exposure-time', str(params.get('initial_exposure_time'))])
        ...
        if result.returncode != 0:
            error_msg = result.stderr if result.stderr else "Unknown slicing error"
            return False, f"SLA slicing failed: {error_msg}"

        # Check if output file was created
        if not os.path.exists(output_path):
            return False, "SLA file was not created"
        ...
        return False, f"SLA slicing error: {str(e)}"
...
def extract_resin_usage(sla_path):
    """
    Extract resin usage and print time from SLA/DLP slice file (.sl1/.sl1s)
    PrusaSlicer SLA output is a zip file containing config.ini with metadata

    Args:
        sla_path: Path to .sl1 or .sl1s file

    Returns:
        dict: Dictionary containing resin usage and time information
    """
    import zipfile
    import configparser
    ...
    try:
        ...
        # .sl1 files are zip archives containing config.ini
        if zipfile.is_zipfile(sla_path):
            with zipfile.ZipFile(sla_path, 'r') as zf:
                # Read config.ini from the archive
                if 'config.ini' in zf.namelist():
                    with zf.open('config.ini') as config_file:
                        ...
                        # usedMaterial is in ml
                        if config.has_option('DEFAULT', 'usedMaterial'):
                            resin_volume_ml = float(config.get('DEFAULT', 'usedMaterial'))
                        
                        # printTime is in seconds
                        if config.has_option('DEFAULT', 'printTime'):
                            estimated_time_seconds = int(float(config.get('DEFAULT', 'printTime')))
                        
                        # numFast + numSlow = total layers
                        num_fast = int(config.get('DEFAULT', 'numFast', fallback='0'))
                        num_slow = int(config.get('DEFAULT', 'numSlow', fallback='0'))
                        layer_count = num_fast + num_slow

        # Fallback values if parsing fails
        if resin_volume_ml is None:
            resin_volume_ml = 10.0  # Default 10ml
        if estimated_time_seconds is None or estimated_time_seconds == 0:
            estimated_time_seconds = 3600  # Default 1 hour
        if layer_count is None:
            layer_count = 100  # Default guess

        return {
            'resin_volume_ml': round(resin_volume_ml, 2),
            'resin_weight_g': round(resin_volume_ml * 1.1, 2),  # Typical resin density
