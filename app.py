...
from utils import allowed_file, convert_stl_to_gcode, convert_stl_to_sla, extract_filament_usage, extract_resin_usage
...
def health_check():
    ...
    return jsonify({
        'status': 'healthy',
        'version': config.get('application', 'version', default='1.0.0'),
...
def analyze_stl():
    ...
    Supports both FDM (--export-gcode) and DLP/SLA (--export-sla)
    Technology is detected from material type or explicit 'technology' parameter
    """
    ...
    # Explicit technology override (optional)
    technology = request.form.get('technology', '').lower()
    ...
    # Detect technology from material if not explicitly provided
    if not technology:
        technology = material_config.get('type', 'fdm').lower()
    
    if technology not in ['fdm', 'dlp', 'sla']:
        return jsonify({'success': False, 'error': f'Invalid technology: {technology}. Use fdm, dlp, or sla'}), 400

    # Normalize sla to dlp (same slicing process)
    if technology == 'sla':
        technology = 'dlp'

    # Validate quality (for FDM) or use DLP defaults
    quality_config = config.get('print_quality', quality)
    if not quality_config and technology == 'fdm':
        return jsonify({'success': False, 'error': f'Invalid quality: {quality}'}), 400

    # Set default printer based on technology if not provided
    if not printer:
        printer = 'prusa_mk3s' if technology == 'fdm' else 'elegoo_mars'
    
    printer_config = config.get_printer(printer)
    if not printer_config:
        return jsonify({'success': False, 'error': f'Invalid printer: {printer}'}), 400

    # Validate infill for FDM
    if technology == 'fdm':
        try:
    ...
    # Prepare slicing parameters based on technology
    if technology == 'fdm':
        params = {
            'layer_height': quality_config.get('layer_height', 0.2) if quality_config else 0.2,
        ...
        # DLP/SLA parameters
        params = {
            'layer_height': material_config.get('layer_height', 0.05),
            'exposure_time': material_config.get('exposure_time_s', 8),
            'initial_exposure_time': material_config.get('bottom_exposure_s', 60),
            'support': support
        }
    ...
    try:
        ...
        # Set output extension based on technology
        if technology == 'fdm':
            output_ext = ".gcode"
        else:
            output_ext = ".sl1"
        ...
        app.logger.info(f"Processing STL file: {filename} (technology: {technology})")

        # Get slicer path
        slicer_path = config.get_slicer_path()

        # Slice based on technology
        if technology == 'fdm':
            success, error = convert_stl_to_gcode(input_path, output_path, params, slicer_path)
        else:
            success, error = convert_stl_to_sla(input_path, output_path, params, slicer_path)

        if not success:
            app.logger.error(f"Slicing failed: {error}")
            return jsonify({'success': False, 'error': error}), 500

        # Extract usage info based on technology
        if technology == 'fdm':
            usage_info = extract_filament_usage(output_path)
            usage_info['technology'] = 'fdm'
        else:
            usage_info = extract_resin_usage(output_path)
            usage_info['technology'] = 'dlp'

        if 'error' in usage_info:
            app.logger.error(f"Failed to extract usage info: {usage_info['error']}")
            return jsonify({'success': False, 'error': usage_info['error']}), 500
